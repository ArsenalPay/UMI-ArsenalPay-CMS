...
<xsl:stylesheet	version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="purchasing[@stage = 'payment'][@step = 'arsenalpay']">
		...
		<script>
			...
			element: "app-widget",
			destination: "]]><xsl:value-of select="destination" /><![CDATA[",
			widget: "]]><xsl:value-of select="widget" /><![CDATA[",
			amount: "]]><xsl:value-of select="amount" /><![CDATA[",
			userId: "]]><xsl:value-of select="userId" /><![CDATA[",
			nonce: "]]><xsl:value-of select="nonce" /><![CDATA[",
			widgetSign: "]]><xsl:value-of select="widgetSign" /><![CDATA["
			});
			APWidget.render();
			]]>
		</script>
	...
	<xsl:template match="purchasing[@stage = 'payment'][@step = 'invoice']" xmlns:xlink="http://www.w3.org/TR/xlink">
		...
		<script>
			jQuery(document).ready(function() {
				...
				site.forms.emarket.toggleNewObjectForm('#invoice', '#new-legal-person');
			});
		</script>
